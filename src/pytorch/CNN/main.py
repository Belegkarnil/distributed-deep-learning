...
def getConfiguration():
    ...
    argParser.add_argument("-l", "--nlayers",    dest='N_LAYER',   type=int, nargs='?', default=2, help="Number of dense blocks")
    argParser.add_argument("-s", "--size",    	dest='SIZE',   type=int, nargs='?', default=4, help="BatchNorm size in dense layers")
...
def worker(model,out_device,criterion,optimizer,epochs,trainset,validationset,testset,sync,lrDecay,verbose=False):
...
def process(rank,world,backend,config):
    ...
    #
    model			= Model(dense_layers=config['N_LAYER'],bn_size=config['SIZE'],devices=devices,pipeline_size=pipeline_size)
    model			= torch.compile(model)
    criterion	= torch.nn.CrossEntropyLoss()
    optimizer	= torch.optim.SGD(model.parameters(),lr=0.01,momentum=0.9)
    lrDecay		= torch.optim.lr_scheduler.StepLR(optimizer, step_size=7, gamma=0.1)
    #
    ...
    #
    worker(model,out_device,criterion,optimizer,config['EPOCHS'],trainset,validationset,testset,sync,lrDecay,rank==0)
    #
    if(torch.distributed.is_initialized()):
        torch.distributed.barrier()

def main():
    # TODO compile pytorch with USE_TENSORRT
